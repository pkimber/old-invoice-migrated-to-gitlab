...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_column(u'invoice_invoice', 'date_created')

        user_pk = 1
        if not db.dry_run:
            # find a default user for updating new columns
            for u in orm['auth.User'].objects.all():
                user_pk = u.pk
                break

        # Adding field 'Invoice.user'
        db.add_column(u'invoice_invoice', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=user_pk, to=orm['auth.User']),
                      keep_default=False)

        # Adding field 'InvoiceLine.user'
        db.add_column(u'invoice_invoiceline', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=user_pk, to=orm['auth.User']),
                      keep_default=False)
    ...
    def backwards(self, orm):
        ...
        # User chose to not deal with backwards NULL issues for 'Invoice.date_created'
        raise RuntimeError("Cannot reverse this migration. 'Invoice.date_created' and its values cannot be restored.")

        # The following code is provided here to aid in writing a correct migration        # Adding field 'Invoice.date_created'
        db.add_column(u'invoice_invoice', 'date_created',
                      self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, blank=True),
                      keep_default=False)

        # Deleting field 'Invoice.user'
        db.delete_column(u'invoice_invoice', 'user_id')

        # Deleting field 'InvoiceLine.user'
        db.delete_column(u'invoice_invoiceline', 'user_id')
